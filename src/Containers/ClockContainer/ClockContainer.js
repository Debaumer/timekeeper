...
class ClockContainer extends Component {
  ...
  completeTaskHandler = () => {
    if (this.state.seconds === 0 && this.state.minutes === 0 && this.state.hours === 0) {
      ...
      this.props.onPostTasks(this.getToday(), finishedTask, this.props.token, this.props.userId);
    }
  }
  ...
  render() {
    ...
    if(!this.props.userId)
      warningMessage = <p>This Won't Save to the server if you don't log in, numbnuts.</p>;
...
const mapDispatchToProps = dispatch => {
  return {
    onPostTasks: (today, task, token, userId) => dispatch(actions.postTasks(today, task, token, userId)),
